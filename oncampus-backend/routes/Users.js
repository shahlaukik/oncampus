...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const destination = `oncampus/users/${newUser._id}/profile_picture`;
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            // Destination for profile_picture
            const destination = `oncampus/users/${user_id.toString()}/profile_picture`;
            ...
            if (req.body.room_number)
                profile.room_number = req.body.room_number;
    ...
    try {
        ...
        if (user)
            return res.status(401).send({
                message:
                    "There's an account associated with this email. Proceed to login.",
            });
...
router.post("/reset-password", async (req, res) => {
    try {
        if (!req.body.reset_request_id)
            return res
                .status(400)
                .send({ message: "Reset Request ID is required" });
