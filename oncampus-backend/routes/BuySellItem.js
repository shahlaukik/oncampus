...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const destination = `oncampus/users/${newProduct.posted_by}/buy-sell/${newProduct._id}`;
        ...
        try {
            ...
            if (
                ...
                return res
                    ...
                    .send({
                        message:
                            "You are not authorized to access this link. Contact the admin if tou think this is a mistake.",
                    });
            ...
            // Destination for the files
            const destination = `oncampus/users/${product.posted_by}/buy-sell/${product._id}`;

            if (currentFiles + toUpload - toDelete < 1)
                return res.status(400).send({
                    message:
                        "Your product should have atleast one image in it.",
                });
...
router.delete("/delete-buy-sell-product", UserAuth, async (req, res) => {
    try {
        ...
        await DeleteAFolder(
            `oncampus/users/${product.posted_by}/buy-sell/${product._id}`
        );
