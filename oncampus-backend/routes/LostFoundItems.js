...
router.get("/get-lost-found-product-details", UserAuth, async (req, res) => {
    try {
        ...
        if (!req.query.product_id)
            return res
                .status(400)
                .send({ message: "Product ID Required" });
    ...
    async (req, res) => {
        try {
            ...
            if (req.body.files.length > 0) {
                ...
                const destination = `oncampus/users/${newProduct.posted_by}/lost-found/${newProduct._id}`;
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            if (
                product.posted_by.toString() !==
                req.body.user_details._id.toString()
            )
                return res.status(401).send({ message: "You are not authorized to access this link. Contact the admin if tou think this is a mistake." });
            ...
            if (toUpload > 0) {
                // Destination for the files
                const destination = `oncampus/users/${product.posted_by}/lost-found/${product._id}`;
...
router.delete("/delete-lost-found-product", UserAuth, async (req, res) => {
    try {
        ...
        await DeleteAFolder(
            `oncampus/users/${product.posted_by}/lost-found/${product._id}`
        );
...
router.put("/mark-as-found", UserAuth, async (req, res) => {
    try {
        ...
        if (product.found_by_someone)
            return res
                .status(200)
                .send({ message: "Product is already marked as found" });
        ...
        return res.send({ message: "Product marked as found" });
    } catch (error) {
        return res.status(500).send({ message: "Server Error. Please try again Later" });
    }
});
